...
function ResearchContent() {
  ...
  useEffect(() => { localStorage.setItem("activeListId", activeListId); }, [activeListId]);
  ...
  function removeFromWatchlist(t: string, listId: string) {
    setWatchlists(prev => prev.map(list =>
      list.id === listId ? { ...list, tickers: list.tickers.filter(x => x !== t) } : list
    ));
  }
  ...
  function renameWatchlist(id: string, name: string) {
    setWatchlists(prev => prev.map(list => list.id === id ? { ...list, name } : list));
  }
  ...
  return (
    <div className="flex overflow-hidden" style={{ height: "calc(100vh - 96px)", background: "var(--background)" }}>
      {/* Sidebar */}
      <aside className="w-56 flex-shrink-0 flex flex-col border-r py-4 px-3 gap-3 overflow-hidden"
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="flex items-center gap-4 px-6 py-3.5 border-b flex-shrink-0"
          style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
          <SearchBar onSearch={t => { setTicker(t); addToWatchlist(t); }} />
        </header>
          ...
          {ticker ? (
            ...
            <div className="flex flex-col items-center justify-center h-full gap-6">
              ...
              <div className="text-center flex flex-col gap-1.5">
                <div className="text-xl font-bold" style={{ color: "var(--text)" }}>Research any stock</div>
              ...
              <div className="flex flex-col items-center gap-2">
                ...
                <div className="flex gap-2 flex-wrap justify-center">
                  {QUICK_PICKS.map(t => (
                    <button key={t} onClick={() => { setTicker(t); addToWatchlist(t); }}
                      className="px-3.5 py-1.5 rounded-lg text-xs font-bold hover:opacity-80"
