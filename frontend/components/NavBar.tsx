...
import { TrendingUp, Palette, Check, BarChart2, Briefcase, Zap } from "lucide-react";
...
export default function NavBar() {
  ...
  return (
    <nav className="flex items-center justify-between px-5 flex-shrink-0"
      style={{ height: 56, background: "var(--surface)", borderBottom: "1px solid var(--border)", zIndex: 50 }}>
      ...
      <div ref={ref} style={{ position: "relative" }}>
        <button onClick={() => setOpen(o => !o)} title="Change theme"
          className="flex items-center gap-2 px-3 h-8 rounded-lg text-xs font-semibold"
        ...
        {open && (
          <div className="absolute right-0 mt-2 rounded-xl overflow-hidden"
            style={{ width: 180, background: "var(--surface)", border: "1px solid var(--border2)", boxShadow: "0 8px 24px var(--shadow)", zIndex: 100 }}>
            ...
            {THEMES.map(t => (
              <button key={t.id} onClick={() => { setTheme(t.id as Theme); setOpen(false); }}
                className="w-full flex items-center gap-3 px-3 py-2.5 text-sm hover:opacity-80"
