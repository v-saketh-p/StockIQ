...
# Provider config — Cerebras primary, Groq fallback
AI_PROVIDERS = [
    {
        "name":    "Cerebras",
            ...
            try:
                with httpx.stream(
                    ...
                    if resp.status_code != 200:
                        ...
                        last_error = f"{provider['name']} error {resp.status_code}: {body[:200]}"
                        continue
                ...
                last_error = f"{provider['name']} exception: {str(e)}"
                continue

        if last_error:
            yield f"data: {json.dumps({'text': '<p style=color:red>' + last_error + '</p>'})}\n\n"
