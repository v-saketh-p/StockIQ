...
Data sourced from yfinance. AI analysis generated by a local model — verify all figures independently. DYOR.
    ...
    def stream_html_report():
        # 1. Send Python-generated header immediately (no AI wait)
        yield f"data: {json.dumps({'text': header_html})}\n\n"

        # 2. Stream Ollama analysis HTML chunks
        try:
            with httpx.stream(
                "POST",
                ...
                if resp.status_code != 200:
                    yield f"data: {json.dumps({'text': '<p style=color:red>Ollama error: ' + str(resp.status_code) + '</p>'})}\n\n"
                else:
